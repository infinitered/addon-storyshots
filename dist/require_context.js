...
function getFullPath(dirname, request) {
  if (isRelativeRequest(request) || !_global.process.env.NODE_PATH) {
    return _path2.default.resolve(dirname, request);
  }

  return _path2.default.resolve(_global.process.env.NODE_PATH, request);
}
...
function runWithRequireContext(content, options) {
  ...
  newRequire.context = function (directory) {
    ...
    var fullPath = getFullPath(dirname, directory);
